...
export class TestTable {
    ...
    transform(node) {
        ...
        const args = node.args.map((arg) => arg.range.source.text.slice(arg.range.start, arg.range.end));
        const testName = args[0];
        const expectationTemplate = args[1];
        const rawValues = node.args[args.length - 1]
            .elementExpressions;
     ...
     * @returns The test.
     */
    static generateTest(name, expect) {
     ...
     */
    static hydrateTemplate(template, values, iValue) {
        ...
        while (instantiation.search(/row[0-9]/) > -1) {
            ...
            if (match === null || match.length < 2) {
                ...
                const replacement = values[iValue];
